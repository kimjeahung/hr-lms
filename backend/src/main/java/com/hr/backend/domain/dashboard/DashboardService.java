...
public class DashboardService {
    ...
    public DashboardResponse getDashboard() {
        ...
        Map<String, List<Enrollment>> byDept = enrollments.stream()
                ...
                .collect(Collectors.groupingBy(e -> e.getUser().getDepartment()));
        ...
        // 마감 임박 알림 (마감일 있는 강의, 오늘 이후)
        DateTimeFormatter fmt = DateTimeFormatter.ofPattern("yyyy.MM.dd");
        List<DashboardResponse.DeadlineAlert> alerts = courses.stream()
                .filter(c -> c.getDeadline() != null && !c.getDeadline().isBefore(LocalDate.now()))
                .sorted(Comparator.comparing(Course::getDeadline))
                .limit(5)
                .map(c -> {
                    long notDone = enrollments.stream()
                            .filter(e -> e.getCourse().getCourseId().equals(c.getCourseId()))
                            .filter(e -> e.getStatus() != Enrollment.Status.DONE)
                            .count();
                    return DashboardResponse.DeadlineAlert.builder()
                            .courseId(c.getCourseId())
                            .title(c.getTitle())
                            .deadline(c.getDeadline().format(fmt))
